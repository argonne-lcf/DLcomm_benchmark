...
class ConfigValidator:
    ...
    def validate(self, cfg: DictConfig, mpi_rank: int, log):
        ...
        if collective not in self.spec["collective"]:
            if mpi_rank == 0:
                log.error(f"[VALIDATION] Invalid collective '{collective}'. Valid: {self.spec['collective']}")
            has_errors = True
        ...
        if op not in valid_ops:
            if mpi_rank == 0:
                log.error(f"[VALIDATION] Invalid op '{op}' for collective '{collective}'. Valid: {valid_ops}")
            has_errors = True
        ...
        if scale_up_algo not in valid_algos:
            if mpi_rank == 0:
                log.error(f"[VALIDATION] Invalid scale_up_algorithm '{scale_up_algo}' for collective '{collective}'. Valid: {valid_algos}")
            has_errors = True
        ...
        if scale_out_algo not in valid_algos:
            if mpi_rank == 0:
                log.error(f"[VALIDATION] Invalid scale_out_algorithm '{scale_out_algo}' for collective '{collective}'. Valid: {valid_algos}")
            has_errors = True
        ...
        if dtype not in self.spec["dtype"]:
            if mpi_rank == 0:
                log.error(f"[VALIDATION] Invalid dtype '{dtype}'. Valid: {self.spec['dtype']}")
            has_errors = True
